      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-2 z-50 w-32 bg-white dark:bg-black border-[0.5px] border-gray-200 dark:border-gray-700 shadow-lg">
            {THEME_ORDER.map(opt => (
              <button key={opt} onClick={() => { setTheme(opt); setIsOpen(false); }}
                className={`w-full px-4 py-2 text-left text-[10px] font-mono uppercase tracking-wider flex items-center gap-2 border-b-[0.5px] border-gray-100 dark:border-gray-800 last:border-0 hover:bg-[#0047FF] hover:text-white ${theme === opt ? 'bg-[#0047FF] text-white' : 'text-gray-600 dark:text-gray-300'}`}>
                {THEME_ICONS[opt]} {t(`theme.${opt}`)}
              </button>
            ))}
          </div>
        </>
