...
export const fetchCategoryById = async (id: number): Promise<CategoryVo> => {
  return get<CategoryVo>(`${BASE_PATH}/${id}`);
};

/**
 * Fetch category by slug
 */
export const fetchCategoryBySlug = async (slug: string): Promise<CategoryVo> => {
  return get<CategoryVo>(`${BASE_PATH}/slug/${slug}`);
};
...
export const createCategory = async (request: CategoryUpsertRequest): Promise<CategoryVo> => {
  return post<CategoryVo, CategoryUpsertRequest>(BASE_PATH, request);
};
...
export const updateCategory = async (request: CategoryUpsertRequest): Promise<CategoryVo> => {
  return put<CategoryVo, CategoryUpsertRequest>(`${BASE_PATH}/${request.id}`, request);
};
