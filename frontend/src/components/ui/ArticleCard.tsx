  ...
  return (
    <article className={`group border-gray-200 dark:border-gray-800 py-6 md:py-2 transition-colors duration-200 ${className}`}>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-[10px] font-mono uppercase tracking-wider text-gray-500 dark:text-gray-400">
        {displayIndex && (
          <span className="w-8 text-gray-400 dark:text-gray-500">[{displayIndex}]</span>
        )}
        <span>{formatDate(article)}</span>
        {categoryName && (
          <>
            <span className="text-gray-300 dark:text-gray-600">|</span>
            <span className="px-2 py-0.5 border-[0.5px] border-gray-200 dark:border-gray-700">
              {categoryName}
            </span>
          </>
        )}
      ...
      {/* Summary */}
      {article.summary && (
        <p className="mt-2 text-sm text-gray-600 dark:text-gray-300 leading-relaxed line-clamp-2 max-w-2xl">
          {article.summary}
        </p>
      )}
    </article>
